...
import { AUTHORS_QUERY } from "@/lib/apollo/admin_queries";
...
const Authors = ({
  ...
  /* use query */
  const [getAuthors, { data, loading, error }] = useLazyQuery(
    AUTHORS_QUERY,
    {}
  );
  ...
  useEffect(() => {
    ...
    const { authors = [], author_aggregate } = { ...data };
    const { aggregate } = { ...author_aggregate };
